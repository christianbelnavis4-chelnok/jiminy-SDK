...
**Design partners only** — this step needs a per-tenant HMAC signing key (`SECRET_HMAC_KEYS`), which is provisioned by your operator, not available to self-serve tenants. A self-serve trace without a signing key evaluates correctly but reports `trace_integrity: unverified`, not `broken` — that's expected, not an error.
